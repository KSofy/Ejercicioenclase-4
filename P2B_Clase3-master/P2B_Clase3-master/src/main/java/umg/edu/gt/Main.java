...
public class Main {
    ...
    private static void aplicarDescuentoAPedido() {
        ...
        if (pedido != null) {
            DescuentoAplicable descuento = new DescuentoPorcentaje(porcentaje);
            pedido.aplicarDescuento(descuento);
            System.out.println("Nuevo Total: " + pedido.getTotal());
        } else {
            System.out.println("Error, no se encontro ningun pedido con el ID " + pedidoId);
        }
